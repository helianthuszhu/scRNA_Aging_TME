...
outDir <- file.path("output8_cellchat_s1/")
...
for (i in 1: length(unique(sce_sel$age_group))){
    ...
    aa= sce_sel
    #
    Idents(aa)='Defined_Cell_Subtype_function'
    levels(aa)
    ...
    cellchat <- createCellChat(object = data.input, meta = meta_target, group.by = "Defined_Cell_Subtype_function")
    #
    cellchat <- setIdent(cellchat, ident.use = "Defined_Cell_Subtype_function") # set "labels" as default cell identity
...
###
    saveRDS(cellchat, file = paste0(outDir, '/',ageindex[i], '_cellchat_output_ctypedetailed.rds'))
