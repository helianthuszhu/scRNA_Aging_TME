...
setwd('/data/Zhuxq/young_LC_analysis/my_analysis4_merge.Pubdata.org/output2_step2.5.3.filter_junk_scanpy_pipeline/data_Fibroblasts_v2')
library(zellkonverter)
mysce=readH5AD('adata_Fibroblasts_v2_degs_signature_louvain_recluster_renamed_subtype_Defined_bad_cluster_rm_agegrouped_generanked.h5ad')
#
library(Seurat)
sce=  as.Seurat(mysce, counts = "counts", data = "logcounts")

saveRDS(sce, file= 'adata_Fibroblasts_v2_degs_signature_louvain_recluster_renamed_subtype_Defined_bad_cluster_rm_agegrouped_generanked.zellkonverter.rds')
...
datadir='output2_step2.5.3.filter_junk_scanpy_pipeline/data_Fibroblasts_v2/'
figdir='output2_step2.5.3.filter_junk_scanpy_pipeline/figure_Fibroblasts_v2/'
...
adata <- sc$read_h5ad(paste0(datadir, 'adata_Fibroblasts_v2_degs_signature_louvain_recluster_renamed_subtype_Defined_bad_cluster_rm_agegrouped_generanked.h5ad'))
adata
...
#
saveRDS(sce, file= paste0(datadir, 'adata_Fibroblasts_v2_degs_signature_louvain_recluster_renamed_signature.4s_added_subtype_Defined_bad_cluster_rm_generanked.rds'))
#############
######
...
###
#library(Seurat)
#sce= readRDS(paste0(datadir, 'adata_Fibroblasts_v2_degs_signature_louvain_recluster_renamed_signature.4s_added_subtype_Defined_bad_cluster_rm_generanked.rds'))
#
...
#dim(statpath)
#############
statpath= read.csv(paste0(datadir, '/adata_Fibroblasts_v2_degs_signature_louvain_recluster_renamed_subtype_Defined_bad_cluster_rm_agegrouped_generanked_fibfunctionsignatureadded.csv'), header=T, row.names=1)
dim(statpath)
###
###
library(ggplot2)
...
ggplot(statpath)+
  aes(x = age_group, y = 	KEGG_VASCULAR_SMOOTH_MUSCLE_CONTRACTION, fill = age_group) + # add color to boxes with fill
...
ggplot(statpath)+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),legend.position="right",
        ...
        panel.background = element_blank())
ggplot(statpath)+
  aes(x = age_group, y = 	KEGG_ANTIGEN_PROCESSING_AND_PRESENTATION, fill = age_group) + # add color to boxes with fill
  #geom_boxplot(varwidth = FALSE) + # vary boxes width according to n obs.
  geom_boxplot(aes(fill=age_group),outlier.colour = "black",outlier.size = 1)+
  #geom_jitter(alpha = 0.8, width = 0,size=1) + # adds random noise and limit its width
  facet_wrap(~Defined_Cell_Subtype_function, nrow=1) + # divide into 2 panels
...
library(ggplot2)
library(patchwork)
pdf(paste0(figdir, '/boxplot_signatures.ageGroup_vs_celltype_generally2.pdf'),width=9,height = 5)
...
prostat_long$Defined_Cell_Subtype_function= factor(prostat_long$Defined_Cell_Subtype_function, 
     levels=c('C1-CAF-COL13A1','C2-CAF-COL14A1',
              'C3-CAF-CTHRC1',
              'C4-myCAF-MYH11',
              'C5-Pericyte-RGS5'))
...
tindex= c('C1-CAF-COL13A1','C2-CAF-COL14A1',
              'C3-CAF-CTHRC1',
              'C4-myCAF-MYH11',
              'C5-Pericyte-RGS5')
plist= list()
...
library(patchwork)
pdf(paste0(figdir, '/Cor_age_vs_cellPercentage2.indi.pdf'),width=20,height = 4)
...
setwd('/data/Zhuxq/young_LC_analysis/my_analysis4_merge.Pubdata.org')
datadir='output2_step2.5.3.filter_junk_scanpy_pipeline/data_Fibroblasts_v2/aged_degs_seurat/'
if(!dir.exists(datadir)) dir.create(datadir,recursive=TRUE)
figdir='output2_step2.5.3.filter_junk_scanpy_pipeline/figure_Fibroblasts_v2/aged_degs_seurat/'
if(!dir.exists(figdir)) dir.create(figdir,recursive=TRUE)
#######
library(Seurat)
sce = readRDS(paste0('output2_step2.5.3.filter_junk_scanpy_pipeline/data_Fibroblasts_v2/', 'adata_Fibroblasts_v2_degs_signature_louvain_recluster_renamed_signature.4s_added_subtype_Defined_bad_cluster_rm_generanked.rds'))
sce
#######
#sce <- AddMetaData(
 # object = sce,
...
cell_type= c('C1-CAF-COL13A1'=color_CLASS[1],'C2-CAF-COL14A1'=color_CLASS[2],'C3-CAF-CTHRC1'=color_CLASS[3],
             'C4-myCAF-MYH11'=color_CLASS[4],'C5-Pericyte-RGS5'=color_CLASS[5])
             )
#
...
sce
#
datadir='output2_step2.5.3.filter_junk_scanpy_pipeline/data_Fibroblasts_v2/aged_degs_smooth/'
if(!dir.exists(datadir)) dir.create(datadir,recursive=TRUE)
figdir='output2_step2.5.3.filter_junk_scanpy_pipeline/figure_Fibroblasts_v2/aged_degs_smooth/'
if(!dir.exists(figdir)) dir.create(figdir,recursive=TRUE)
#
######
#sce <- AddMetaData(
...
cname2=c('C1-CAF-COL13A1','C2-CAF-COL14A1',
              'C3-CAF-CTHRC1',
              'C4-myCAF-MYH11',
              'C5-Pericyte-RGS5'
             )
...
################load data
#library(Seurat)
setwd('/data/Zhuxq/young_LC_analysis/my_analysis4_merge.Pubdata.org/output4_step4.0.scenic.allcells.py/output_Fibroblasts')

figdir <- file.path("figure_Fibroblasts/")
if(!dir.exists(figdir)) dir.create(figdir,recursive=TRUE)

datadir <- file.path("data_Fibroblasts/")
if(!dir.exists(datadir)) dir.create(datadir,recursive=TRUE)
...
cname=c('C1-CAF-COL13A1','C2-CAF-COL14A1',
              'C3-CAF-CTHRC1',
              'C4-myCAF-MYH11',
              'C5-Pericyte-RGS5'
             )
...
library(ggplot2)    
#save_pheatmap_pdf(g, paste0(figdir, '/degs_age_dependent_smooth_merged.pdf'),width =16,height = 16)
ggsave(paste0(figdir, '/TFs_age_dependent_smooth_initial_clusters_merged.pdf'),g,width =15,height = 6) 
#################
#################
...
figdir <- file.path("figure_Fibroblasts/reduced/")
if(!dir.exists(figdir)) dir.create(figdir,recursive=TRUE)

datadir <- file.path("data_Fibroblasts/reduced/")
if(!dir.exists(datadir)) dir.create(datadir,recursive=TRUE)
...
down_cluster=c(3,5)
i=1
    load(paste0('data_Fibroblasts/',cname2[i],'_data_for_subheatmap_draw.RData'))
    gsel= rownames(subset(annotation_row, Cluster %in% c(up_cluster,down_cluster )))
    lenupg= length(rownames(subset(annotation_row, Cluster %in% up_cluster )))
    lendowng= length(rownames(subset(annotation_row, Cluster %in% down_cluster )))   
...
library(ggplot2)    
#save_pheatmap_pdf(g, paste0(figdir, '/degs_age_dependent_smooth_merged.pdf'),width =16,height = 16)
ggsave(paste0(figdir, '/TFs_age_dependent_smooth_merged_reduced.pdf'),g,width =15,height = 6) 
...
i=3
    load(paste0('data_Fibroblasts/',cname2[i],'_data_for_subheatmap_draw.RData'))
    gsel= rownames(subset(annotation_row, Cluster %in% c(up_cluster)))
    lenupg= length(rownames(subset(annotation_row, Cluster %in% up_cluster )))
    #lendowng= length(rownames(subset(annotation_row, Cluster %in% down_cluster )))   
    ...
    pdf(paste0(figdir, cname2[i], '_age_pheatmap_clusters_reduced_UP_part.pdf'))
    add.flag(ph,kept.labels = gene_name,repel.degree = 0.5)
    dev.off()
#    
...
head(enrich_merged)
enrich_merged$celltype= factor(enrich_merged$celltype, levels=c('C1-CAF-COL13A1','C2-CAF-COL14A1',
              'C3-CAF-CTHRC1',
              'C4-myCAF-MYH11',
              'C5-Pericyte-RGS5'))
...
write.csv(gene_list_merged, file= paste0(datadir,'Fibroblast_age_dependent_DEGs_list_merged.csv'))
#####
